...
/*
  Packet Queue used by EAMRP.
*/

eamrp_rqueue::eamrp_rqueue() {
  head_ = tail_ = 0;
  len_ = 0;
  limit_ = EAMRP_RTQ_MAX_LEN;
  timeout_ = EAMRP_RTQ_TIMEOUT;
}

void
eamrp_rqueue::enque(Packet *p) {
...
Packet*
eamrp_rqueue::deque(nsaddr_t dst) {
Packet *p, *prev;
...
char 
eamrp_rqueue::find(nsaddr_t dst) {
Packet *p, *prev;  
...
void
eamrp_rqueue::findPacketWithDst(nsaddr_t dst, Packet*& p, Packet*& prev) {
...
bool
eamrp_rqueue::findAgedPacket(Packet*& p, Packet*& prev) {
